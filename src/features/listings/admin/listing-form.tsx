    ...
    <div className="mx-auto w-full max-w-[1320px]">
      ...
      >
        ...
        <ListingFormSection description="Cover image first, followed by the detail-page carousel." icon={ImageIcon} title="Images">
          <ListingImageManager error={errors.images} initialImages={listing?.images ?? []} onError={(message) => pushToast("error", message)} />
        </ListingFormSection>
        ...
        <div className="flex flex-col-reverse gap-2 bg-[#fffdf8] p-5 sm:flex-row sm:items-center sm:justify-end sm:p-6 lg:p-8">
          <Link className="inline-flex h-11 items-center justify-center rounded-lg border border-[#d9d5c9] bg-white px-5 text-sm font-black text-slate-600 transition hover:bg-slate-50" href="/listings">
            Cancel
          </Link>
          <button className="inline-flex h-11 min-w-36 items-center justify-center gap-2 rounded-lg bg-[#C0964E] px-5 text-sm font-black text-[#123C36] shadow-[0_14px_28px_rgba(192,150,78,0.22)] transition hover:bg-[#A87E36] disabled:cursor-wait disabled:opacity-70" disabled={isPending} type="submit">

...
export function CmsTableRouteSkeleton() {
  return (
    <section
      ...
      className="overflow-hidden rounded-[14px] border border-slate-200 bg-white shadow-[0_18px_48px_rgba(15,23,42,0.09)]"
    >
      <div className="overflow-visible border-b border-[#0f6f7d]/45 bg-[#123342] px-4 py-3 text-white max-[640px]:px-3">
        <div className="flex min-h-12 min-w-0 flex-wrap items-center gap-2 max-[640px]:grid max-[640px]:grid-cols-1">
          <SkeletonBlock className="h-9 w-28 rounded-lg bg-white/18 max-[640px]:w-full" />
          <div className="flex min-h-9 min-w-0 flex-[1_1_420px] items-center overflow-hidden rounded-lg border border-white/18 bg-white/92 shadow-[0_10px_24px_rgba(0,0,0,0.12)] max-[900px]:w-full max-[640px]:min-h-10">
          ...
          <SkeletonBlock className="h-9 w-28 rounded-lg bg-white/18 max-[640px]:w-full" />
          <SkeletonBlock className="h-9 w-32 rounded-lg bg-white/18 max-[640px]:w-full" />
        </div>
      </div>

      <div className="overflow-x-auto bg-white max-[680px]:hidden">
        <div className="min-w-[980px]">
          <div className="grid grid-cols-[1.1fr_1fr_1fr_1fr_0.8fr_88px] border-b border-slate-200 bg-[#f3f7ff]">
            {Array.from({ length: 6 }, (_, index) => (
              <div
                className="flex min-h-9 items-center border-r border-slate-200/90 px-3 last:border-r-0"
                key={index}
              >
          ...
          <div className="bg-white">
            {tableRows.map((row) => (
              <div
                className={cn(
                  "grid min-h-[35px] grid-cols-[1.1fr_1fr_1fr_1fr_0.8fr_88px] border-b border-slate-100 last:border-b-0",
                  row % 2 === 0 ? "bg-white" : "bg-[#f8fbff]",
                )}
                key={row}
              >
                {Array.from({ length: 6 }, (_, cell) => (
                  <div
                    className="flex min-w-0 items-center border-r border-slate-100 px-3 last:border-r-0"
                    key={`${row}-${cell}`}
                  >
                    {cell === 5 ? (
